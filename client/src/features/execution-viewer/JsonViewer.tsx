...
function JsonValue({ value, depth, expandAll, wrap }: { value: unknown; depth: number; expandAll: boolean | null; wrap: boolean }) {
  ...
  const visibleEntries = expanded && !expandAll ? entries.slice(0, ARRAY_PREVIEW_LIMIT) : entries;
  ...
  return <span className="inline-block min-w-0 align-top">
    ...
    {!expanded ? <span className="text-zinc-500">{isArray ? '[' : '{'} {suffix} {isArray ? ']' : '}'}</span> : <span>
      ...
      <span className="ml-4 block border-l border-zinc-200 pl-2 dark:border-zinc-700">
        {visibleEntries.map(([key, child]) => <span key={key} className={wrap ? 'block whitespace-pre-wrap break-words' : 'block whitespace-nowrap'}><span className="text-sky-700 dark:text-sky-400">{isArray ? '' : `${JSON.stringify(key)}: `}</span><JsonValue value={child} depth={depth + 1} expandAll={expandAll} wrap={wrap} /></span>)}
        {entries.length > visibleEntries.length && <span className="block text-zinc-500">… {entries.length - visibleEntries.length} additional items (use Expand all)</span>}
      </span>
      <span className="text-zinc-500">{isArray ? ']' : '}'}</span>
    </span>}
  </span>;
...
export function JsonViewer({ value }: { value: unknown }) {
  ...
  return <div className="mt-1 rounded-md border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-950">
    <div className="flex flex-wrap items-center gap-1 border-b border-zinc-100 px-2 py-1.5 text-[10px] dark:border-zinc-800">
      ...
      <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search JSON" aria-label="Search within result" className="w-28 rounded border border-zinc-200 bg-transparent px-1.5 py-0.5 text-[10px] outline-none focus:border-blue-500 dark:border-zinc-700" />
      {query && <span className="text-zinc-500">{matches}{matches >= 100 ? '+' : ''}</span>}
      <button type="button" onClick={() => setExpandAll(false)} aria-label="Collapse all JSON" className="rounded px-1 hover:bg-zinc-100 dark:hover:bg-zinc-800">Collapse all</button>
      <button type="button" onClick={() => setExpandAll(true)} aria-label="Expand all JSON" className="rounded px-1 hover:bg-zinc-100 dark:hover:bg-zinc-800">Expand all</button>
    ...
    <pre className="max-h-none overflow-x-auto p-2 font-mono text-[10px] leading-relaxed text-zinc-700 dark:text-zinc-300"><JsonValue value={safeValue} depth={0} expandAll={expandAll} wrap={wrap} /></pre>
  </div>;
}
